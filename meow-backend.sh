#!/usr/bin/env bash
# =============================================================================
#  Meow Backend — one-line VPS installer
#  Installs Xray-core and configures a WebSocket backend that Meow's
...
#     bash <(curl -fsSL https://raw.githubusercontent.com/aqakoohyar/Tools/main/meow-backend.sh)
#
#  Or with a custom path/port (port MUST be Cloudflare-allowed: 8080, 8880, 2052, 2082, 2086, 2095):
#     MEOW_PATH=/mysecret MEOW_PORT=8080 bash <(curl -fsSL .../meow-backend.sh)
#
#  RECOMMENDED — match your Meow panel's UUID so clients work immediately:
#     MEOW_PORT=<your-panel-uuid> bash <(curl -fsSL .../meow-backend.sh)
...
MEOW_PATH="${MEOW_PATH:-/meowvpn}"
# IMPORTANT: Cloudflare Workers can only fetch() to a fixed set of ports. For plain HTTP the
# allowed ports are 80, 8080, 8880, 2052, 2082, 2086, 2095. Meow relays to this backend FROM a
# Worker, so the backend MUST listen on one of those — otherwise the relay silently times out.
# Defaults: VLESS on 8080, VMess on 8880 (both Cloudflare-allowed). Override VLESS with MEOW_PORT.
MEOW_PORT_VLESS="${MEOW_PORT:-8080}"
MEOW_PORT_VMESS="${MEOW_PORT_VMESS:-8880}"
[ "${MEOW_PATH:0:1}" = "/" ] || MEOW_PATH="/$MEOW_PATH"
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  echo "   sudo bash <(curl -fsSL .../meow-backend.sh)"
  exit 1
fi
...
say "Meow Backend installer starting"
echo "    path : ${c_bld}${MEOW_PATH}${c_rst}"
echo "    ports: VLESS ${c_bld}${MEOW_PORT_VLESS}${c_rst}  |  VMess ${c_bld}${MEOW_PORT_VMESS}${c_rst}"
echo
...
# ---- 2) UUID: use MEOW_PORT if provided (to MATCH your panel), else generate -
# IMPORTANT: the UUID here MUST match your Meow panel's UUID, or clients get dropped.
# Easiest: copy your panel UUID and run:  MEOW_PORT=<panel-uuid> bash <(curl ... meow-backend.sh)
if [ -n "${MEOW_PORT:-}" ]; then
  UUID="$MEOW_PORT"
...
cat > "$XRAY_CONFIG" <<JSON
{
  ...
  "inbounds": [
    {
      "tag": "vless-ws",
      "port": ${MEOW_PORT_VLESS},
      "listen": "0.0.0.0",
      "protocol": "vless",
      "settings": { "clients": [ { "id": "${UUID}" } ], "decryption": "none" },
      "streamSettings": { "network": "ws", "security": "none", "wsSettings": { "path": "${MEOW_PATH}" } }
    },
    {
      "tag": "vmess-ws",
      "port": ${MEOW_PORT_VMESS},
      "listen": "0.0.0.0",
      "protocol": "vmess",
...
if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "Status: active"; then
  ...
  ufw allow "${MEOW_PORT_VLESS}/tcp" >/dev/null 2>&1 || true
  ufw allow "${MEOW_PORT_VMESS}/tcp" >/dev/null 2>&1 || true
  ok "Firewall ports opened"
else
  warn "ufw not active — make sure ports ${MEOW_PORT_VLESS}/${MEOW_PORT_VMESS} are reachable (cloud provider firewall too)."
fi
...
# ---- 8) print the summary ----------------------------------------------------
MEOW_BACKEND_URL="http://${PUBIP}:${MEOW_PORT_VLESS}${MEOW_PATH}"
echo
echo "${c_grn}${c_bld}════════════════════════════════════════════════════════════════${c_rst}"
echo "${c_grn}${c_bld}  ✅ Meow Backend is ready${c_rst}"
echo "${c_grn}${c_bld}════════════════════════════════════════════════════════════════${c_rst}"
echo
echo "  ${c_bld}1) In the Meow panel → Network & IPs → Backend mode:${c_rst}"
echo "       • Enable backend mode"
echo "       • Backend URL:  ${c_cyn}${MEOW_BACKEND_URL}${c_rst}"
echo "       • Save"
echo
echo "  ${c_bld}2) Your details (keep them safe):${c_rst}"
echo "       UUID : ${c_cyn}${UUID}${c_rst}"
echo "       Path : ${c_cyn}${MEOW_PATH}${c_rst}"
echo "       VMess port (if you use VMess): ${c_cyn}${MEOW_PORT_VMESS}${c_rst}"
echo
echo "  ${c_bld}3) Client config (VLESS) — point it at your MEOW DOMAIN, not this IP:${c_rst}"
echo "     ${c_cyn}vless://${UUID}@YOUR-MEOW-DOMAIN:443?security=tls&type=ws&host=YOUR-MEOW-DOMAIN&sni=YOUR-MEOW-DOMAIN&path=$(printf '%s' "$MEOW_PATH" | sed 's;/;%2F;g')&encryption=none#meow-backend${c_rst}"
echo
echo "  ${c_yel}Flow:${c_rst} client → Cloudflare/Meow (TLS) → this VPS (Xray) → internet"
echo "  ${c_yel}Note:${c_rst} Meow's 'Health check' may say no /health — that's normal; test with a real client."
echo "  ${c_yel}Ports:${c_rst} this uses ${c_bld}${MEOW_PORT_VLESS}${c_rst}/${c_bld}${MEOW_PORT_VMESS}${c_rst} because Cloudflare Workers can only relay to a"
echo "         fixed set of ports (80,8080,8880,2052,2082,2086,2095 for http). Open ${MEOW_PORT_VLESS} in your"

#!/usr/bin/env bash
# =============================================================================
#  meow Backend — connectivity tester
#  Checks, from the OUTSIDE, whether your Xray backend is reachable and speaks
#  WebSocket on the path meow will forward to. Run it from your laptop (best,
#  proves the firewall is open to the world) or from the VPS itself.
#
#     bash <(curl -fsSL https://raw.githubusercontent.com/aqakoohyar/Tools/main/meow-backend-test.sh) 76.13.79.199 10000 /meowvpn
#
#  Args:  <VPS_IP>  <PORT>  <PATH>      (defaults: 10000  /meowvpn)
# =============================================================================
set -uo pipefail
...
IP="${1:-}"; PORT="${2:-10000}"; WPATH="${3:-/meowvpn}"
[ "${WPATH:0:1}" = "/" ] || WPATH="/$WPATH"
if [ -z "$IP" ]; then
  echo "Usage: $0 <VPS_IP> [PORT] [PATH]"
  echo "   e.g: $0 76.13.79.199 10000 /meowvpn"
  exit 1
fi

echo "${c_bld}meow backend tester${c_rst}"
...
hd "2) Does it speak WebSocket on ${WPATH}? (this is what meow forwards)"
...
if printf '%s' "$RESP" | grep -qi "101"; then
  ...
  echo "     /usr/local/etc/xray/config.json (default ${c_bld}/meowvpn${c_rst})."
...
hd "Result"
if [ "$PASS" = "1" ]; then
  ok "${c_bld}Backend looks GOOD.${c_rst} meow can forward to it."
  echo
  echo "  Next: in a client (Hiddify), import your VLESS link pointed at your meow DOMAIN"

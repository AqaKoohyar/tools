...
## 🇬🇧 English — consumed by Meow Panel "Smart (per-ISP)" mode

meow Radar (https://github.com/aqakoohyar/radios) writes its scan results here, **one file per
...
### File format
Plain text, one entry per line — exactly meow Radar's "Save to .txt" export:
```
104.18.42.137:443
...
1. Open **meow Radar**, pick the source(s), scan, let it verify (TCP+TLS) and sort by latency.
...
## 🇮🇷 فارسی — مصرف‌شده توسط حالت «هوشمند (هر اپراتور)» پنل Meow

برنامهٔ meow Radar (https://github.com/aqakoohyar/radios) نتایج اسکنش را اینجا می‌نویسد، **برای هر
...
### فرمت فایل
متن ساده، هر خط یک ورودی — دقیقاً همان خروجی «ذخیره در .txt» در meow Radar:
```
104.18.42.137:443
[2606:4700::6812:2a89]:2053
...
۱. **meow Radar** را باز کن، منبع(ها) را انتخاب کن، اسکن کن، بگذار تأیید (TCP+TLS) و بر اساس تأخیر
   مرتب کند.
